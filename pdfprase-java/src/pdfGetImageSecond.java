...
public class pdfGetImageSecond {
    ...
    public static void main(String[] args)  {
       ...
       // List<String>fileTxtSave=new ArrayList<>();//要存放的提取出来的txt位置
        for(int i=0;i<filePathBefore.size();i++){
            ...
            filePathMid.add(filePathBefore.get(i)+File.separator+"pdfforimag");
            System.out.println(filePathBefore.get(i)+File.separator+"pdfforimag");
            ...
            while(getImagAndTxtFlag){
                ...
                for(int j=0;j<splitPdf.size();j++){
                   ...
                   // System.out.println(splitPdf.get(j)+"  "+splitFileName.get(j)+"  "+fileTxtSave.get(i)+"  "+j);
                    ...
                    System.out.println(f.delete());//等这个图片的结束后再删除
                }
            }
            saveEndFile(filePathBefore.get(i)+File.separator+"imag"+File.separator+"end.txt");
        }
    }
